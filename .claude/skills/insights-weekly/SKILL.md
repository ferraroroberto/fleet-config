...
Post the digest **as the caption of the dated report file** — phone push shows the at-a-glance summary *and* the full markdown attached to open on mobile. **Activity-log** traffic, so route with `--category log` (helper resolves the `coding log` chat from `hooks/projects.toml` — never hardcode a channel id). Pass the **absolute** report path `report.py` printed on line 1 to `--file`, pipe the digest body via stdin (carries emoji / em-dash / bullet cleanly — `notify_send` decodes stdin as UTF-8):

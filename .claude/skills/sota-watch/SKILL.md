...
One `AREA=` line per area — `due` (research now), `fresh` (report "next due <date>" in one line, nothing else), or `delegated` — plus a `DUE=` total. A typical run has 0–2 due areas; a run where everything is fresh still posts the digest (staleness relay + next-due table is the product, not a failure).

...
**This skill files no issues itself.** The only writes are (a) the audit issues each sub-agent's `/codebase-audit` files, (b) the per-repo `audit-meta` ledger those audits update, (c) one `audit-fleet digest state` ledger issue in `fleet-config`, (d) the digest comment on that issue, and (e) one cross-fleet `fleet practices ledger` issue in `project-scaffolding`. The orchestrator never edits source, commits, pushes, or restarts anything. **One narrow exception lives inside the sub-agents:** `/codebase-audit`'s step 8b self-heals a **security** finding in place (redacted issue + auto-fix + auto-merge) — the only code-writing path in the whole flow, gated on its own safety rules (claim, mandatory regression test, generic artifacts, green-gate-only merge). This orchestrator just carries the counts-only result into the digest.

**Designed for unattended runs.** A weekly app-launcher job invokes the co-located `run-weekly.bat`, which routes `/audit-fleet` plus its Sonnet/high-effort/bypass-permissions flags through the shared `claude_progress.py` stream adapter. Every step must therefore degrade gracefully rather than block on a prompt. The orchestrator only does cheap enumeration/gating/dispatch, so it runs at `easy` tier; the per-repo sweep sub-agents dispatched in step 3 run at `hard` tier (`docs/model-tiers.md`).
...
- **Shell:** the Bash tool here is **Git Bash**. `gh` and `git` work identically
  in it. Do not use PowerShell syntax (`&`, `$env:`, here-strings) in Bash.
  Windows paths map as `/e/automation/...`.
- **The orchestrator only does cheap, safe work:** enumeration, the per-repo
  ledger gate, fast-forward syncs, windowed dispatch, collection, the digest.
  **All file reading happens inside sub-agents** — this is what keeps the
...
- **Never end the turn to "wait for it."** This orchestrator runs headless via
  `run-weekly.bat`'s one-shot Claude process — no persistent turn loop, no human
  attending, no wake-up mechanism after a turn ends. Launching anything with
  `run_in_background: true` and then ending the turn silently kills the entire
  run: the CLI exits on that clean turn-end reporting `exit_code: 0` (false
  success) while nothing past that point ever happened (`fleet-config#314`).
  - This extends to a tool call the *harness itself* auto-backgrounds past its
    own timeout ceiling. An instruction cannot prevent an *automatic*
    timeout-background, so the fix is structural: **never let a single call span
    the sweep's real runtime at all** (`fleet-config#609` — the step 2 sweep ran
    345s–1460s+, past the Bash tool's 600s ceiling; see step 2).
  - Every command here — including the step 3 rate-limit pause — must run
    synchronously (foreground) or poll to completion within the same turn
    against a concrete, externally observable condition (e.g. the `Monitor`
    tool's until-loop pattern for step 3's rate-gate pause), never
    fire-and-forget.
  - **Prose alone isn't enough even for the poll itself:** a model-composed
    `Monitor` wait returned without actually blocking and the turn ended
    believing a notification would arrive (`fleet-config#609`, reopened). Step 2
    therefore uses a foreground blocking helper script, not a model-composed
    wait.

## Self-pacing against the live session budget

A full fleet sweep can exhaust the rolling **5-hour session rate limit** mid-run.
`statusline-command.ps1` caches the live `rate_limits.five_hour` usage % (+
`resets_at`) to `~/.claude/hooks/state/rate-limits.json` on every render
(fleet-config#259), so this skill pauses dispatch proactively, waits in place,
and resumes within the same run — no relaunch, no OS-level scheduling, no
`resume` argument; a run always ends by delivering one full digest. Mechanics:
step 3. Full design: `docs/rate-gate.md`.
...
orchestrator reads its JSON, never runs a per-repo LLM loop. But the sweep runs
345s–1460s+ and grows with the fleet, crossing the Bash tool's 600s ceiling, and
pinning one tool call to that runtime stranded a run for 10 hours with zero
digest delivered (`fleet-config#609`). So: launch the sweep **detached** and poll
a **sentinel file** — a concrete, externally observable condition, not an opaque
harness-tracked background task with nothing for `Monitor` to watch.
...
`fleet_audit_scan.py` mints a fresh, unique one per invocation
(`E:/tmp/fleet-audit-scan-<pid>-<ns>.json` when `E:/tmp` exists, the system temp
dir otherwise), so two overlapping runs (a manual retry alongside a still-running
scheduled one) can never collide. The detached child is the same `scan()` this
step always ran, spawned `CREATE_NEW_PROCESS_GROUP | NO_WINDOW` (the same
survive-the-parent pattern `hooks/restart_and_verify_webapp.py` uses) so it
outlives this tool call, and it publishes its JSON to `SWEEP_RESULT` atomically
(temp-file-then-rename, so a poller never observes a partial write) — or an
`{"error": "..."}` payload if the sweep raises, so a crash is distinguishable
from "still running" instead of leaving a poller waiting for a file that will
never appear.

**Wait** for `SWEEP_RESULT` with a foreground blocking helper script — never
`Monitor`, never a raw backgrounded Bash call: a `Monitor`-composed until-loop
returned in about a second instead of blocking, and the run ended its turn
awaiting a notification `claude -p` never delivers (`fleet-config#609`,
reopened). `skills/_lib/wait_for_sentinel.py` blocks in a real synchronous sleep
loop inside a single Bash call and returns an unambiguous, machine-checkable
answer:
...
and continue below with its JSON. **Exit 2** (`SENTINEL-NOT-READY <path>`) — not
a "wait for a notification" signal; it is a "call this exact command again right
now, in this same turn" signal, so re-invoke it immediately.
`--timeout-seconds 560` keeps a single call comfortably under the 600s ceiling,
so the retry is a normal part of the design, not a failure. **Cap retries at 26**
(≈4 hours, comfortably above every observed historic duration including the
1460s outlier). If `SWEEP_RESULT` still hasn't appeared after 26 exit-2 results,
or it appears carrying an `{"error": ...}` payload, treat this as a
pre-flight-class failure: print `Fleet audit plan — sweep did not complete:
<reason>` and skip straight to step 6's delivery assertion, which correctly finds
nothing to report and prints `SCHEDULED-RUN-FAILED` — never fabricate a plan line
from a scan that never finished. **Never end this turn on an exit-2 result** —
the only two places this step may stop polling are a resolved exit 0 or an
exhausted retry cap.
...
that lost two repos from three consecutive runs (fleet-config#567). Never report
...
week, until a human clears it. A stale lock is invisible to every read
(`status`, `fetch`, `rev-list` and an up-to-date `pull --ff-only` all exit 0 with
the right answer) while the repo is frozen against every write, so nine locked
repos were filed as healthy `below_threshold` and nobody noticed for fifteen days
(fleet-config#667). `verdict: stale` means no git process is running on the
machine at all; `stale_unconfirmed` means the lock is past the threshold but that
could not be established — both need a look, neither is auto-repaired. **Never
delete a lock from this skill**, and never instruct a sub-agent to: it is another
process's file. The fix is a human confirming the holder is dead, then removing
it.

A `to_audit` entry carrying a `reason` is **not** organic change — it is a repo
the gate was *forced* to audit because it could not read the ledger
(`fleet_audit_scan.broken_ledgers()` returns exactly these; don't re-derive the
filter in prose). `unresolvable-baseline`: the recorded `last-audited-sha`
(`baseline_sha`) resolves to nothing in the checkout, almost always a
squash-merged, deleted feature-branch tip. `unparseable-ledger`: the ledger issue
(`ledger_issue`) carries no readable `<!-- audit-ledger` block at all. Auditing
whole-repo is the safe answer to both, so they are correctly in `to_audit` — but
each re-bills a full Opus whole-repo pass *every week* until the ledger is
repaired, so surface them by name in the plan line and the digest rather than
letting them read as ordinary churn. Both self-heal once the repo's own audit
reaches step 9, since every ledger write normalizes the block.
...
`<!-- audit-self-fix -->` comment on its ledger issue — no further write needed
here.

A `below_threshold` entry is the opposite of a silent skip: the repo has real
organic (non-self-fix) commits since its last audit, but their weighted-LOC
significance (`skills/_lib/audit_issue.py`'s `unexplained_weighted_loc` —
feature/refactor count fully, docs/test count nothing, fix/chore count partially)
hasn't crossed the threshold yet. Its ledger sha is **not** advanced, so next
week's check covers the same (growing) commit range plus whatever lands
meanwhile; when the total finally justifies a whole-repo audit it moves to
`to_audit` and that audit reaches back to the ledger sha, so nothing is ever
lost. If the single-repo argument was passed, `--only <name>` restricts the whole
sweep to it.
...
(`fleet-config#642`: skipped repos dropped from the report, so work went missing
while the run read clean). A new reason (`index-lock in flight`, added in #667)
must reach the operator by name the day it first fires, without this file being
edited again — which is why the line is rendered from the reason the sweep
actually returned rather than from a vocabulary written down here.
...
- **`OK` / `UNKNOWN`** → dispatch the next repo. Fill the window with up to 3
  background `Agent` calls (`run_in_background: true`,
  `subagent_type: "general-purpose"`, `model: "opus"`), then **stay in this same
  turn and block on `TaskOutput` (`block: true`) for every task now in flight** —
  never end the turn to "wait for it" (see Execution rules): an unpolled
  background task is silently killed at the CLI's 600s background-task ceiling
  and the run reports a false `exit 0` (`fleet-config#506`, the same class of gap
  `fleet-config#314` closed for this skill's own Bash/Monitor calls). If a
  `TaskOutput` call times out before a task finishes, re-issue the same blocking
  call — never move on with a task still unresolved. As each task returns, record
  its report and immediately dispatch the next repo from the to-audit list to
  refill the window — never more than **3 in flight**, and the turn must never
  end while any task is still dispatched. Fewer than 3 repos left → dispatch just
  that many. No git worktrees: `/codebase-audit` is read-only and only files
  issues, so agents in different repo directories cannot collide.
...
run-log counts, never recomputed) — that is what lets step 5's digest separate
genuinely new findings from standing backlog instead of treating "an issue got
upserted this run" as the unit of "new."
...
`OK` — refill the window the moment a slot frees. The whole loop runs inside one
turn — block on `TaskOutput` for the in-flight window, refill on each return,
repeat until the to-audit list is drained; the turn never ends with a sub-agent
still dispatched (`fleet-config#506`).

### 4. Collect results

Hold each sub-agent's structured report as it returns. When the to-audit list is
drained with no agent still in flight (whether or not pause cycles happened),
proceed to the practices ledger (4b) then the digest. Track two terminal buckets,
plus the `self_fix`, `below_threshold`, `skipped`, and `stale_lock` buckets
already decided by step 2's Python sweep (carried through unchanged — no
sub-agent touches those repos):
...
the cross-fleet "things that work" ledger (assets to remember, not rot to fix),
labelled `audit-meta` so `/issue-triage` filters it out.
...
design baseline — note it as the initial design snapshot rather than a delta.
...
with two fleet-wide fetches (read-only, no repo reading), via the direct Issues
API rather than `gh search issues --owner`, which is backed by GitHub's Search
API and was observed reporting issues as open for five-plus weeks after they had
actually closed (fleet-config#623):
...
Group each result by `repository.name` into per-repo open counts. This is the one
place the fleet's design-drift accounting is tallied week-over-week — one ledger,
alongside the six code buckets, never conflated with them: `/audit-fleet` is the
unified reporter, `/design-sweep` the doer that keeps those issues current. If
either fetch fails outright, note `design-drift: count skipped (<reason>)`, carry
the last ledger snapshot forward unchanged, and never fail the run over it. If a
fetch's stderr summary reports per-repo `ERROR` lines but still returns a partial
result, use the partial count and note which repos were skipped rather than
discarding the whole tally.
...
- **Security section** *(only when any sub-agent reported a non-`NONE`
  ...
  `<repo>: 1 gap ESCALATED — branch left for manual /issue-finish`. The
  vulnerability, file, and class live nowhere in the digest, the ledger, or any
  commit. An `ESCALATED` line is the digest's durable flag that a security gap
  needs a human.
- **Self-fix section** *(only when non-empty)*: repos step 2's sweep classified
  ...
  closed #71, #64 — ledger advanced, no organic change`), so it's visible they
  were deliberately *not* re-audited rather than silently skipped.
...
- **Skipped section:** repos skipped for dirty / off-branch / non-ff /
  index-lock-in-flight, so they read as intentionally left out, not silently
  missed.
- **Stale-lock section** *(only when non-empty)*: repos carrying a stranded
  ...
  write; needs a human to confirm the holder is dead and remove it`). Never
  resolved by the run itself, and it repeats verbatim until acted on — that
  repetition is the point (fleet-config#667).
- **Session-limit section** *(only when non-empty)*: repos left unaudited
  because the step-3 3-pause safety net was hit, so they are visibly outstanding
  rather than silently dropped — next week's ledger gate picks them up for free.
- **New findings this week:** built strictly from the `new` counts each
  ...
  where `new > 0`. This is the actual delta, not "an issue got upserted" — list
  these at the top so the email leads with what genuinely changed.
- **Standing backlog:** a single fleet-wide count — the sum of every
  `carried` + `stale` count across every audited repo this run, never an item
  list. One line, e.g. `14 standing findings across 5 repos, unchanged or not
  re-verified this run — see each repo's audit issue for detail.`
...
- **The ledger gate is one shared Python implementation, not prose.** Step 2's
  ...
  `audit_issue.py`'s `evaluate_repo` — exactly one implementation of the
  skip/audit/self-fix decision. Unit-tested independent of `gh`/`git` in
  `tests/test_audit_issue.py`.
- **Read-only on source — except a sub-agent's step-8b security self-heal**
  (redacted issue + auto-fix + auto-merge, gated on claim, mandatory regression
  test, generic artifacts, green-gate-only merge, escalate on failure). Every
  other write is an audit issue, the per-repo ledger, the digest-state issue, the
  digest comment, or the practices ledger in `project-scaffolding` — the one
  issue-write target outside `fleet-config`, and still an issue, never source.
...
- **One sub-agent per repo, `hard` tier (Opus on Claude Code today), through a
  ≤3 sliding window,** refilled as each returns. Widening beyond 3 would need to
  respect the Opus burst-limiter cap (`docs/model-tiers.md`). No worktrees
  (audits don't collide). Don't read repo source in the orchestrator.
- **Block on `TaskOutput` for every in-flight sub-agent, same turn, always.**
  `run_in_background: true` followed by ending the turn is not a valid pattern
  anywhere in this skill (`fleet-config#506`), and step 3's loop never returns
  control until the to-audit list is fully drained. `claude_progress.py` also
  hands the CLI `CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS=0` so an in-flight task is
  waited on rather than killed (`fleet-config#519`) — a safety net against losing
...
- **And the assertion can't save a run that never reaches it.** An orchestrator
  cut off mid-flight never prints the marker at all and the job records
  `success` / exit 0 with zero repos audited. The launcher therefore also passes
  ...
  comment landed within the last 12 hours — the delivered artifact, not a symptom
  (fleet-config#560). Nothing here invokes it, but if step 6's shape ever
  changes, that check changes with it or it starts lying.
...
- **Self-pace against the live session %, don't die-and-hope.** Check
  ...
  via the `Monitor` until-loop and resume — capped at 3 pause cycles per run. No
  OS-level scheduling, no `resume` argument (`docs/rate-gate.md`).
...
- **Why scatter-gather:** each repo's file reading is isolated in its own
  sub-agent context, so the orchestrator never holds the whole fleet's source at
  once — that is what makes a weekly all-repo sweep cheap.
- **Why a ledger gate:** most weeks most repos are unchanged, and the gate turns
  an unchanged repo into one `gh` + one `git` call. The commit SHA is the cache
  key; the rubric hash (sha256 of the repo's **own** project CLAUDE.md) busts one
  repo's cache when its criteria change. The global `~/.claude/CLAUDE.md` is
  deliberately excluded — folding it in re-audited the entire fleet on every
  edit; a fleet-wide re-grade is now an explicit act (clear the ledgers'
  `last-audited-sha`), never a side effect.
- **Self-fix-only churn is treated as unchanged:** `evaluate_repo` detects it via
  merged-PR `closingIssuesReferences` against the managed bucket issues and
  advances the ledger itself — otherwise a repo fixed only via `/cleanup-fleet`
  would be re-flagged weekly (fleet-config#251).
- **A mixed PR fails closed to AUDIT, on purpose** (fleet-config#251 decision
  ...
  recognized as self-fix, so the repo re-audits. Telling them apart would need
...
- **Not every non-self-fix commit re-audits immediately** (fleet-config#315):
  `evaluate_repo` weighs the unexplained commits' `additions + deletions` by
  conventional branch-type (`feat`/`refactor` full weight, `fix`/`chore`
  partial, `docs`/`test` none — `audit_issue.py`'s `PR_TYPE_WEIGHTS`) and only
  audits once the total crosses `DEFAULT_SIGNIFICANCE_THRESHOLD` (1000). Below
  that, `SKIP_BELOW_THRESHOLD` leaves the ledger sha untouched so small organic
  changes batch up rather than forcing a re-audit over one low-risk commit
  (#256) — see step 2.

...
The worktree glob is **per repo, inside the loop, and stays that way**. A fleet-wide `/e/automation/*-wt-*` makes one repo's check report another repo's in-flight worktree as residue (app-launcher#709).
...
**`notify_complete.py` is the ONLY sanctioned way to send these pings — never use an MCP chat tool to find a chat or post it, and a silent no-op is correct, not a bug to route around.** Why: [notify-ping](../../../docs/notify-ping.md).
...
- **Two environment bugs are worked around, neither specific to this skill — re-test if the harness changes:** `scriptPath` invocation (step 7), and `args` arriving as a JSON string rather than a parsed object — `.claude/workflows/cleanup-fleet-all.js` defensively `JSON.parse`s it when it arrives as a string.
- **Before trusting the full unattended schedule**, run one more attended pass covering a bucket with a validator rejection (proves the retry loop, not just the happy path) before wiring `run-weekly.bat` into a scheduled job.
- **Prior incidents this shape encodes** — read before relaxing any rule above: fleet-config#534 (teardown-honesty checks → the two reported-only probes + zombie exception); fleet-config#518 + #515 (parallel fan-out left 11 stray worktrees + two primaries off `main` reporting `0 failed` → serial lanes, Teardown agent, residue halts, forced worktrees, step 8 enumeration); fleet-config#642 (skipped repos dropped not deferred, same shape as #560/#607/#612/#623 → `repo_preflight.py`, 7b retry, `cleanup-deferred` issue); fleet-config#612 (a correct halt that never reached the exit code → `SCHEDULED-RUN-FAILED` marker).

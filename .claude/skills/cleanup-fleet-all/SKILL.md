...
**One bucket at a time, one repo at a time.** Lanes are strictly serial: the next repo does not start until the current lane has run all four agents and its repo is verified clean. At most one worktree exists at any instant. (Issues within a bucket used to fan out in parallel; that concurrency, combined with an escalation path that tore nothing down, is what caused the 2026-07-30 incident below — see Notes.)

...
- **Validated attended once** against the `stale` bucket; all merged first round. Two environment bugs were worked around: `scriptPath` invocation (step 7), and `args` arriving as a JSON string rather than a parsed object — `.claude/workflows/cleanup-fleet-all.js` defensively `JSON.parse`s it when it arrives as a string. Neither is specific to this skill; re-test if the harness changes.

...
**Re-verify state first, in one batch.** Step 3's fetch is not proven immune to every staleness source (see its caveat), and `hard` mode's approval wait can itself take long enough for an issue to close meanwhile. Build one JSON array of every still-selected issue (`[{"repo": ..., "number": ..., ...other fields...}, ...]` — `repo` a bare name like `"task-os"`, never `"owner/name"`: the helper prepends the owner itself, and a prefixed repo produces a doubled-owner `gh` argv that fails with a network-sounding error unrelated to the network, fleet-config#706) and pipe it through:

```
E:/automation/fleet-config/.venv/Scripts/python.exe C:/Users/rober/.claude/skills/_lib/issue_state_gate.py partition
```

Read the stdout JSON directly — `{"dispatch": [...], "skipped_closed": [...], "unresolved": [...]}`. **Only `dispatch` items get a sub-agent below.** For every `skipped_closed` item, drop it and report `already closed — dropped, no agent dispatched`. For every `unresolved` item — the check could not establish open vs. closed (network error, rate limit) — **also drop it from this run**, never guessed as open, and report `state unresolved — dropped, not dispatched (<detail>)` as its own category, never merged into either the closed count or the dispatched count. The run's final summary must show all three counts (`dispatched` / `already-closed` / `unresolved`) even when the latter two are zero — a run that silently shrank its own working set reads as "nothing to do" (fleet-config#623, #560, #612).
...
**Before marking any repo complete, run the post-flight dirty-tree check yourself — never trust the agent's self-report.** The agent reporting the result is the same actor that might have forgotten a commit or left something dirty, so this runs in the orchestrator:

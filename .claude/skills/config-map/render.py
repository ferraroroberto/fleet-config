...
def _file_url(html: Path) -> str:
    # forward-slashed absolute path + force placeholders (parity with system-map)
...
def render(html: Path, out: Path, scale: float = 2.0) -> tuple[int, int]:
    ...
    probe = _run_chrome([
        ...
        "--window-size=400,300", f"--screenshot={tmp / ('cm_probe_' + uuid.uuid4().hex + '.png')}",
        url,
    ])
    ...
    staged = tmp / f"cm_{uuid.uuid4().hex}.png"
    shot = _run_chrome([

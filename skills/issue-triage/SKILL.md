...
- **One fetch call total, and no shell-side post-processing.** Step 2's single helper invocation is the only shell work this skill does — no jq/awk/sed, no second script, no per-repo re-query, no piping its output into another tool. The agent reads that output **directly**; grouping, scoring and picking suggestions are all model-side from there.

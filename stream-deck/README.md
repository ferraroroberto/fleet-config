...
## Deploying a change (automated)

After editing `registry/targets.json` or the plugin code, the `streamdeck-deploy` Claude Code skill (`.claude/skills/streamdeck-deploy/`, fleet-config#591) runs everything that's scriptable — `.env` connection check, `npm run verify`, link-or-restart the live plugin, `npm run package`, and `npm run profile-diff` (diffs the exported profile's actual placed keys against the registry, over a real read of the `.streamDeckProfile` zip) — then reports exactly which targets still need a physical key, instead of you eyeballing the registry against the device. The sections below are the manual/reference version of the same steps.
...
Connections live in the plugin's `.env` (see `.env.sample`): home-automation needs `HOME_AUTOMATION_BASE_URL` + `HOME_AUTOMATION_TOKEN`; facilitation-suite defaults to `http://127.0.0.1:8449` with no token (it trusts loopback) — set `FACILITATION_SUITE_BASE_URL` / `FACILITATION_SUITE_TOKEN` only to point elsewhere.

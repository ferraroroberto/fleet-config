...
- **Hooks refuse through `_lib.block()`** (Claude: exit 2 + stderr; confirmed Codex `PreToolUse`: structured deny + exit 0); non-blocking hooks exit 0 with a single short nudge. Never hand-roll either — `_lib.block()` and `_lib.warn()` own the wire format, which is per-event and per-harness: plain exit-0 stdout only reaches the model on `SessionStart`/`UserPromptSubmit`/`UserPromptExpansion`. Claude `PreToolUse` uses `systemMessage`; Codex `PreToolUse` and `PostToolUse` use `hookSpecificOutput.additionalContext`; other Claude `PostToolUse`-class events use the event's supported additional-context field (fleet-config#681, #745). A nudge printed to a channel the model never reads is a guard that only reports.
...
- **Foreign-harness payloads are translated in exactly one place** — `_lib.normalize_payload()`, called from `read_stdin_json()` (fleet-config#491). A hook must never learn a second harness's key names, event vocabulary, or tool ids; a harness sending something new means extending the maps there. Claude-shaped payloads are returned as the *same object*, asserted by `tests/test_payload_normalization.py`, because `hooks/` is junctioned live into `~/.claude/hooks` — a merge is fleet-wide against running sessions the moment it lands. Likewise `_lib.block()` owns the per-harness refusal dialect (Codex `PreToolUse`: structured deny + exit 0; Claude: exit 2 + stderr; Grok additionally needs a `deny` decision on stdout, because it reports our 2 as 1 and fails **open** on anything else). A guard that only *reports* a block is worse than no guard: verify a real refusal live in the target harness. Procedure: `docs/adding-a-coding-harness.md`.

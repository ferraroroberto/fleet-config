...
- **Every `subprocess` spawn passes `creationflags=NO_WINDOW`** — `_lib.NO_WINDOW` in `hooks/`, `from no_window import NO_WINDOW` in `skills/_lib/` and `.claude/skills/*` (fleet-config#412). Never re-inline the `sys.platform == "win32"` ternary. This repo's scheduled skills *are* the console-less parent of the global convention. `tests/run_acceptance.py` parses `hooks/`, `skills/`, and `.claude/skills/` and fails on any spawn missing the flag; `tests/` itself is exempt.

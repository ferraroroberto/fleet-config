...
Versioned home for user-scope coding-agent configuration. After `install.ps1`, `hooks/` is visible at `~/.claude/hooks/` (and vice-versa) via a Windows junction — no copy step. This repo's own `global-CLAUDE.md` supplies the global instructions and applies here in full; below is only what's repo-specific.

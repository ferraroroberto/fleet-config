...
It uses normal config discovery, invocation-scoped trust, `--approve-for-me`, and reviewed wrappers under disposable `.codex/hooks/`; it neither copies credentials nor changes live hooks. The feature-branch leg proves a safe multi-file patch runs, the PreToolUse GitHub-body advisory and both PostToolUse advisories reach the model, and a dated docs target is refused. The default-branch leg proves a gitignored control edit runs while a committable target is refused. A final `--ignore-user-config` negative control intentionally creates the dated sentinel, proving that mode suppresses project hooks and therefore cannot count as policy coverage. Pass depends on actual file effects and transcript evidence, not registration alone.

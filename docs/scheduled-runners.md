...
The shared runner requires a valid terminal event and observed tool work, with no malformed or unverifiable records and no pending tools/children. Claude's `result` must establish success; Codex's `turn.completed` must have its native usage object. A prose promise, tool start, or failed terminal result with process exit zero never establishes completion. An unrecognised future record establishes nothing either way: it is counted and named, and decides nothing on its own (fleet-config#810). Known reasoning, usage/status and hook metadata is ignored without exposing raw payloads; tool summaries are allowlisted and secret-redacted. Individual failed tools may recover within the same native run, as the Codex smoke demonstrates. A real artifact still requires the caller's delivery check.

### Known record types, and what an unknown one means

Unknown stays unknown: the verdict policy is not the thing that drifts, the parser's knowledge is. `KNOWN_IGNORED_SYSTEM_SUBTYPES` in `skills/_lib/runner_adapters.py` is the explicit allowlist of Claude `system` subtypes that are ambient state rather than a progress boundary (`thinking_tokens`, `status`, `compact_boundary`, `hook_started`, `hook_response`, `hook_progress`, `background_tasks_changed`, `task_updated`). Anything absent from it is counted and named as unknown, and teaching the parser the new subtype remains the fix.

An unknown record is a statement about the parser, not about the run, so since fleet-config#810 it does **not** fire `122` on its own. It could not: the CLI adds record types routinely, so the previous policy sent every scheduled job on the host red the week a new subtype shipped — `fleet-health-weekly` run `20260909T233001` delivered its digest, its ledger entry and its Telegram ping under `150 unknown stream record(s)` and would still have exited `122`. That is the inverse of the failure this runner exists to prevent: it manufactures a red rather than hiding one, and an alert that cries wolf on a timetable is how a real red gets ignored.

What stays verdict-bearing is evidence that a fact was never *established*, never merely that something was unfamiliar: a missing terminal event, a record the adapter could not parse at all, a burst of unknown records near shutdown with no terminal result (the fleet-config#560 signal, unchanged), and the surfaces `ProgressFormatter._mark_unverified` counts — a Codex collaboration item whose completion cannot be read, a drain deadline reached with a pipe still open or a descendant still running, and a child that never exited inside the terminate timeout. Those get their own log line naming the shapes, and keep `122`:

```
⚠ 1 record(s) left this run unverified · item.completed/collab_tool_call ×1
```
...
| `122` | Missing terminal event, malformed record, shutdown burst, or an unverifiable surface |
...
Both stdout and stderr use nonblocking reads, including partial UTF-8 records. Root exit starts a one-second drain grace; remaining owned processes make an otherwise clean run unfinished (`118`) and are terminated. Remaining or unqueryable ownership also closes the pre-effect retry gate, even if both pipes reached EOF and the parent returned a transient API error. Cancellation/stall stops the owned scope, waits at most two seconds to observe its active count reach zero, and bounds pipe draining. Cancellation is confirmed (`130`) only when termination was observed and both pipes reached EOF; failed/unknown queries, failed termination, or unclosed pipes stay unconfirmed (`114`). Deadline logs name the active count (or unknown) and each pipe's EOF state. When an otherwise clean run leaves owned processes behind, the log also names each one before teardown kills it — `owned descendant still running · pid N · <image> · <command line>`, the command line truncated to 160 characters and secret-redacted, at most eight lines plus an `… and N more` tail — or says `owned descendants could not be named` when the job's member list is unavailable (fleet-config#911). The verdict line itself is unchanged. Final job closure is an ownership-scoped safety net, never evidence that upgrades an unconfirmed result. Windows code uses stdlib `ctypes`, `_winapi` and `msvcrt`, with shared `NO_WINDOW` flags; no additional dependency is required.
...
Since fleet-config#810 that fixture proves the parser's *knowledge*, not the verdict policy: the same stream now reaches `exit 0` even with the subtypes removed from the allowlist, because unknown records no longer decide a run. It is still the regression that keeps the lifecycle read correctly — `task_notification` consumed once per child, the patch deliberately not — which `118` and `120` do still score.

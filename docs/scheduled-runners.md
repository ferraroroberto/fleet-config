...
Cancellation is supported through `KeyboardInterrupt` or `run_process(cancel_event=...)`. On Windows, `process_scope.py` creates a private kill-on-close Job Object, launches a waiting Python bootstrap, assigns it to the job, and only then releases the native command. Failure to establish ownership aborts before provider code runs. The retained job owns ordinary CreateProcess descendants after the native parent exits; cancellation never looks up an exited parent PID or targets process names/ports. POSIX keeps a separate process group.

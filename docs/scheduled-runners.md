...
Cancellation is supported through `KeyboardInterrupt` or `run_process(cancel_event=...)`. It stops only the owned PID tree (`taskkill /T /F /PID` on Windows; a new process group on POSIX), waits for the child and drains its stderr. Windows subprocesses use the shared `NO_WINDOW` flag. Failure to establish tree termination is its own unconfirmed outcome. Arbitrary external hard-killing of the Python wrapper is not a verified graceful-cancellation path; it cannot promise cleanup. No process names, ports, live PTYs or unrelated process trees are targeted.

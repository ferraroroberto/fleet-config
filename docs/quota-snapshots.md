...
App-server is experimental, so the collector currently admits only the locally probed CLI 0.153.3 and 0.153.4. The 0.153.4 probe confirmed the same `rateLimitsByLimitId` bucket identities and `primary`/`secondary` `windowDurationMins`, `usedPercent`, and `resetsAt` fields consumed by the adapter. Other versions publish unsupported/client_version_unverified without spawning app-server. Patch releases remain individually allowlisted: accepting a same-minor range would turn an unprobed native shape into apparently valid quota data. Adding a version requires repeating the native read-only probe, inspecting its response shape, and updating the sanitized fixtures/version set. Unknown adapter or on-disk schema versions are rejected. Claude's native statusline quota subset is unversioned; its shape is validated field by field rather than asserting an unavailable CLI version. Verified native producer versions do not certify future versions.

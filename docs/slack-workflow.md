...
slack_notify_user    = "U0123ABCD"   # user id, only @mention'd when slack_notify_mention is on
...
**Board deep link (fleet-config#242).** When `board_url` is also configured (project override, or `[global] board_url` fallback — same shape as `slack_notify_channel`), the ping appends a second line: `📋 <https://<host>:8445/?board=<session_id>|Open on the Board>`. `<session_id>` is Claude's transcript UUID — the same id `session_state.py` already persists as the board row's key. `board_url` must be a Tailscale-reachable address (e.g. `https://<pc>.<tailnet>.ts.net:8445`), not loopback, since the ping is tapped on the phone. Unset by default (the current `hooks/projects.toml` has no live value), so this is a no-op until you configure it — deliberately: the board resolves `?board=<sid>` by matching a card's **session-host id**, but this hook only ever knows the **transcript UUID**, and those two id spaces don't intersect yet. app-launcher#307 adds the resolver (`state_sid`) that bridges them; until it ships, a tapped link toasts "session not on the board". Set `board_url` only after #307 merges.

**No history read, no completion-suppression machinery.** An earlier design kept the idle nag but suppressed it for 30 minutes after a completion ping, by reading the channel back (`conversations.history`) to spot a fresh terminal mark. Dropping `idle_prompt` wholesale (above) made that whole subsystem dead, so it's gone — along with the bot's dependency on the `channels:history`/`groups:history` scope. The bot now needs only `chat:write`. Nothing reads channel history anymore.

...
```bash
py ~/.claude/hooks/slack_notify.py --channel C0123ABCD --text "✅ [project] <message>"
py ~/.claude/hooks/slack_notify.py --channel C0123ABCD --text "🔔 [project] urgent" --mention   # force a tag
```
...
py ~/.claude/hooks/notify_complete.py --kind recap  --summary "3 skills swept — alt-text +2"   # 🔄 Weekly recap — automatic sweep (no proposals)
...
**Only the `permission_prompt` push fires; `idle_prompt` no-ops.** The `Notification` event carries two sub-types: `permission_prompt` (a permission gate *or* an `AskUserQuestion` — the mid-task "come look, I'm blocked" push) and `idle_prompt` (the 💤 "gone idle" nag). The hook pings on the first and **no-ops on the second** — a session you left idle is rarely something you need to run back to, so that nag was noise. A `permission_prompt` is reworded to `Claude awaits your input` (it's as often an `AskUserQuestion` as a real permission gate, so "needs your permission" overclaims). It can't say *what* Claude is waiting on: in a remote-control / bridge session the tool being gated lives in the cloud transcript, and the local `transcript_path` holds only bridge metadata (no `tool_use`), so a question (`AskUserQuestion`) and a real permission gate are indistinguishable locally. Don't add transcript-tool-sniffing here expecting it to work from the phone — it won't.

...
**Keep every `--summary` / `--text` pure ASCII** (emoji aside). A Windows command line is not a UTF-8-safe channel end to end: the harness → shell → `CreateProcess` leg re-encodes, and a literal `·` in a design-sweep summary reached Slack as `??` (fleet-config#507 — reproduced: a BOM-less UTF-8 command handed to Windows PowerShell 5.1 is decoded with the ANSI codepage, so `·`'s two UTF-8 bytes arrive as `Â·`, and a further narrowing to an OEM codepage turns that pair into `??`). Spell a multi-part summary's separator with the ASCII token `|` — `notify_complete.normalize_summary()` renders it as `·` from a **Python source literal**, the same reason the leading emoji and the em-dash always survived. Recoverable mojibake is repaired on the way in (`_lib.repair_mojibake`, also applied to `slack_notify --text`), but a boundary that already replaced the character with `?` has destroyed it — hence the token, not just the repair. `--text` gets no token expansion: its body carries markdown, where `|` is a table cell. The acceptance matrix fails on any `SKILL.md` that authors non-ASCII punctuation into one of these arguments.

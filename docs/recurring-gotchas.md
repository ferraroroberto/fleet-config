...
## A fleet-wide `git status` sweep strands 0-byte index locks unless it sets `GIT_OPTIONAL_LOCKS=0`

`git status` takes `.git/index.lock` purely to write back a refreshed **stat cache** — an optimisation, not part of producing the output. Measured on this host, and narrower than it is usually stated: of `status --porcelain=v2 --branch`, `diff --name-only HEAD`, `diff --name-only --cached HEAD`, `ls-files --others`, `rev-parse --verify` and `symbolic-ref`, **only `status` takes it** — so a fleet reader that never runs `status` is not a creator, and the `(and diff, and friends)` phrasing carried in older docstrings overstates the surface. `status --porcelain=v2 --branch` holds the lock for **4–6 ms starting ~75% of the way through a ~30 ms run**, on **40 of the fleet's 42 repos**. Kill the process inside that window and it leaves exactly a **0-byte `index.lock`** and nothing else touched. With `GIT_OPTIONAL_LOCKS=0` the lock is **never created at all** — while a real write still takes the real index lock and still refuses when one is held, which is correct and must stay (`tests/test_git_run.py` drives both halves).

**Why it is worse than it sounds:** a stale lock is **invisible to every read**. `git status --porcelain`, `git fetch`, `git rev-list` and an already-up-to-date `git pull --ff-only` all exit 0 with correct output while the repo cannot `add`, `commit`, `pull` or `stash`. So no sweep reports `UNKNOWN` and no non-zero exit fires. On 2026-08-01 that hid the condition across nine repos for **fifteen days** (fleet-config#667). On 2026-09-20 it recurred across **23 repos stamped to the same instant**: two `git merge --ff-only` calls printed `Updating <old>..<new>` and *failed*, the next command in each chain read the old HEAD, and two tray restarts then verified and reported the **old** `git_sha` as live — twenty minutes believing two merges were deployed when they were not (fleet-config#939).

**The fleet-wide fingerprint is a parallel fan-out, not a loop.** Repo scans start together and run for near-identical durations, so their lock windows **overlap in real time**; one tree-kill in that band strands most of the wave at once, every lock stamped within a single Windows file-time tick. The 2026-09-20 creator was `app-launcher`'s Board/Coding poll — `/api/claude-code/git-status` fanning `git status` across every child of `projects_dir` (which defaults to `E:/automation`) through a `run_git` that passed no `env=`. The one repo left clean was the *slowest* one, whose lock window fell outside the band the other 40 shared — so "most repos, not all" is the expected shape, and **the clean repo is not evidence of an innocent process**.

**The rules.** Every `git` spawn goes through the tier's wrapper — `_lib.run_git` in `hooks/`, `git_run.run_git` / `run_git_bytes` in `skills/_lib/` and `.claude/skills/*` — because that wrapper is where `git_env()`'s `GIT_OPTIONAL_LOCKS=0` lives; a raw `subprocess.run(["git", …])` opts back out invisibly, still exiting 0 with the right answer. `tests/run_acceptance.py`'s `git_wrapper` check enforces this statically, and **any repo that reads git state fleet-wide needs the same gate over its own tree** — the check's blind spot is its tree scope, never its logic. Detection is `skills/_lib/index_lock.py` (five verdicts, `stale` vs `stale_unconfirmed` kept apart); the short-interval tripwire is `hooks/index_lock_sessionstart.py`, which sweeps the whole fleet rather than the session's cwd precisely because the clean repo is where you are most likely to be sitting.

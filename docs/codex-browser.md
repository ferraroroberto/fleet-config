...
- **`APP_REACHABLE` / `APP_UNREACHABLE_HINT`** — printed only with `--check-app` (off by default, so the plan stays deterministic and offline). This is the one capability failure the CLI can determine itself, so it is the one whose remediation it prints; every other code is observed by the agent driving the browser, which looks its message up by code.
- **Capability failures** — each code carries its own actionable remediation (single-sourced in `browser_verify.FAILURES`, one lookup per determined code — the plan never dumps the whole legend for you to classify by hand). They do **not** collapse into one generic error:

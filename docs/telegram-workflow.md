...
**Verify it's actually wired.** `settings.template.json` carries the `Notification` block, but that block reaches your live `~/.claude/settings.json` only by a one-time manual merge (`install.ps1` only prints the reminder) — there's no re-sync, so the live file can silently drift and lose the block (then idle/permission pings just never fire). Confirm with `E:/automation/fleet-config/.venv/Scripts/python.exe -c "import json;print(list(json.load(open(r'C:/Users/rober/.claude/settings.json'))['hooks']))"` — `Notification` must be in the list. After re-adding it, restart Claude Code (or open `/hooks` once) so the harness reloads settings.

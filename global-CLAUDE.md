...
User-level memory loaded into every coding-agent session on this machine. This one file is the single source of truth for **every** coding agent that reads a user-scope context file: Claude Code reads it as `~/.claude/CLAUDE.md`, Codex as `~/.codex/AGENTS.md`, Pi as `~/.pi/agent/AGENTS.md`, and GitHub Copilot CLI as `~/.copilot/copilot-instructions.md` (same file, symlinked into each home by `fleet-config/install.ps1`). Editing it once is live everywhere. Hooks, the statusline, and tool settings stay Claude-Code + Codex-only — the other agents expose no such surface (the per-agent matrix and the documented non-goals live in `fleet-config`'s README). Guidance applies to whichever agent is reading it; the few sections that are genuinely specific to one agent are marked *(… only — skip on other agents)*.

...
check(rubric.version == "1.5.1", "rubric at 1.5.1 (mock-up alignment in 1.1.0, judgment checklist in 1.2.0, #964 type rules in 1.3.0, #996 spec alignment in 1.4.0, #1019 layers + clearance in 1.5.0, #1017 Android pointer + grid rows in 1.5.1)")
...
check("rubric v1.5.1" in page and f"rubric v{out_v['rubric_version']}" in page, "rubric version printed in the header")
for rid in by_id:
...
k1 = _kv(p1)
check(p1.returncode == 0 and (tmp / "evaluate.json").is_file() and k1.get("GRADE") == "F" and k1.get("RUBRIC") == "1.5.1"

...
check(rubric.version == "1.1.0", "rubric bumped to 1.1.0 with the mock-up alignment")
...
check("rubric v1.1.0" in page and f"rubric v{out_v['rubric_version']}" in page, "rubric version printed in the header")
for rid in by_id:
...
k1 = _kv(p1)
check(p1.returncode == 0 and (tmp / "evaluate.json").is_file() and k1.get("GRADE") == "F" and k1.get("RUBRIC") == "1.1.0"

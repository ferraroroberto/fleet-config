...
| `session_state.py` | `UserPromptSubmit` / `Stop` | Persists `~/.claude/hooks/state/sessions-state.json` for the Fleet Board (#91 ↔ app-launcher#164): one row per recent Claude Code session — `project`, `status` (`working` on prompt submit, `needs-you` on stop, `needs-you`/`idle` via the `notify_on_idle` piggyback), `transcript_path`, `cwd`, `updated_at`. Atomic writes, rows >24h pruned, always exits 0; the board only reads the file. Consumers join rows to launcher sessions by normalized `cwd` (the hook `session_id` is the transcript UUID, not the session-host id). |

...
- **`/learning-log`** — the **journey lens**: turn the fleet's *GitHub work stream* (merged PRs + closed issues across every `ferraroroberto` repo since the last run) into a weekly **learning log + forward horizon**. `report.py` gathers via `gh search`, compresses the high-velocity fleet (~25 merged PRs/day) into a compact **per-repo rollup** (counts by conventional-commit type + sampled notable titles, so the whole window fits one bounded prompt), and hands it to the local hub (`127.0.0.1:8000`) to narrate *what shipped, what we learned, what to archive*, **grade last week's horizon** (shipped / slipped / emerged-unplanned) and set the next one. It reads **no source code** — purely GitHub. The durable archive + live horizon live in one canonical `kind=learning` ledger issue (deduped via `skills/_lib/audit_issue.py`, labelled `audit-meta`); each week's narrative is a comment on it; a `📓 Learning log` Slack ping links straight to it. The window anchors to the ledger's `last-run-at` (a missed run widens the next, never drops a week). Deliberately separate from `/audit-fleet` (code), `/system-map` (architecture — cross-linked, not modified) and `/insights-weekly` (usage metrics). Built to run unattended on a weekly app-launcher job.

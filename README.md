...
- **Junctions** for the directory entries. Cross-volume OK, no admin. `hooks/` and `commands/` are each junctioned into **both** `~/.claude` and `~/.codex`, and `skills/` into `~/.claude/skills` + `~/.agents/skills` (Codex+Pi) + `~/.copilot/skills` (Copilot) — so every agent loads the *same live files* and nothing can drift between them.

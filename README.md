...
| `session_state.py` | `UserPromptSubmit` / `Stop` / `SessionEnd` | Persists `~/.claude/hooks/state/sessions-state.json` for the Fleet Board (#91 ↔ app-launcher#164): one row per recent Claude Code session — `project`, `status` (`working` on prompt submit, `needs-you` on stop, `needs-you`/`idle` via the `notify_on_idle` piggyback), `transcript_path`, `cwd`, `updated_at`. `SessionEnd` (clean exit) **deletes** the row so the board stops showing a session the moment it actually ends (#241); a hard kill never fires it, so those rows still age out via the 24h prune. Atomic writes, always exits 0; the board only reads the file. Consumers join rows to launcher sessions by normalized `cwd` (the hook `session_id` is the transcript UUID, not the session-host id). |

`hooks/pi_usage_stats.py` is the Pi usage bridge for Coding stats. It reads Pi's native JSONL sessions under `~/.pi/agent/sessions/` and emits content-free telemetry — agent=`pi`, cwd/project, provider, model, message/tool-call counts, and token/cost totals from each assistant message's `usage` block. Example: `C:/Users/rober/AppData/Local/Python/bin/python.exe hooks/pi_usage_stats.py --json --include-sessions`. This is the source app-launcher (or any fleet stats job) should ingest for Pi instead of scraping terminal transcripts; prompt/response text is deliberately omitted.
...
├── docs/                           # references: slack-workflow, codex-browser, mcp-context-audit, design-system, model-tiers, rate-gate

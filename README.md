...
- **`/audit-fleet`** — scatter-gather wrapper that runs `/codebase-audit` across every `ferraroroberto` repo under `E:\automation\`, skipping the unchanged ones via the ledger gate, auditing the changed ones through a bounded window of up to 3 concurrent sub-agents (a single-message parallel fan-out across the whole fleet trips Anthropic's burst rate limit; the 3-wide cap is the fleet-wide Opus concurrency window defined in the global `CLAUDE.md`), and emitting one diff-based weekly digest (GitHub comment on the digest-state issue + Slack ping + stdout). Built to run unattended on a weekly app-launcher job. Alongside the rot it files, it runs a **second lens on the same read** — a *fleet practices ledger*: each sub-agent surfaces hard-won reusable solutions and generalizable-convention candidates (the inverse of a finding — an asset to remember, not rot to fix), and the orchestrator merges them into one living catalog issue in `project-scaffolding` so the best of each repo is discoverable fleet-wide. **Survives session rate limits** (fleet-config#222): a full sweep can exhaust the rolling 5h session budget mid-run, and the live % is unreadable headless (Claude Code feeds it to the statusline only, never to disk), so instead of polling it arms a **dead-man's switch** before the heavy phase — `skills/_lib/audit_retry.py` registers a one-shot Windows task ~4h out that re-launches the audit as `/audit-fleet resume`; if this process is killed by the limit the task still fires, and the idempotent ledger gate skips already-audited repos so the resume is near-free. A clean finish disarms it; a bounded retry count (3 launches) caps the chain; a fresh weekly run resets it. On the first rate-limit signature it stops dispatching and defers the rest rather than burning budget on calls that will all 429; a cut-short run that has a retry pending stays **quiet** (no duplicate digest/Slack), and only the completing — or retry-cap-reached partial — run emits the digest.

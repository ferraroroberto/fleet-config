...
- **[`docs/skills.md`](docs/skills.md)** — the three skill tiers (global
  ...
  measure/rubric/scoring/judgment/report core with its run ledger, issue
  filing and fleet sweep, and stranded index locks.
...
├── design.rubric.toml              # /design-review rule set (ids, thresholds, severities, penalties, grades, mock-up ids) + the [[judgment]] checklist a fresh-context judge answers, read by skills/_lib/design_review/ — versioned on its own; runs are diffed by rule id from a local ledger under hooks/state/design-review/ and filed as one managed design-review issue per repo on request (fleet-config#971, #972, #973, #974)
...
│   ├── _lib.py                     # shared hook core (state, spawn flag, git/gh wrappers, payload accessors); re-exports harness_wire.py + registry.py + edit_events.py so no hook's import line changes (fleet-config#819, #931)
│   ├── harness_wire.py             # multi-harness wire protocol: stdin-JSON + normalize_payload inbound, block/warn/rewrite/allow dialects outbound (split from _lib.py, fleet-config#931)
...
├── skills/                         # junction → ~/.claude/skills, ~/.agents/skills (Codex+Pi), ~/.copilot/skills (Copilot) — GLOBAL tier: issue-* workflow, handoff-commit, codebase-audit, design-sync, design-review, screen, _lib/, …
...
**Adding a new hook: wire it live only after the merge.** `~/.claude/hooks` is a junction to the primary checkout, so a hook added on a branch has no module there until the change is on `main` and the primary has pulled it. Wiring the live `settings.json` early makes `run-hook.ps1` print `Hook module not found` on every session start fleet-wide until the merge lands. So the order is: add the module and its `settings.template.json` entry on the branch → merge → pull the primary → copy the entry into the live `settings.json`. While the branch is open, the `settings_sync` acceptance check reports the hook as `PEND` (counted in Skipped, not in Total or Failed) because its module is not in `~/.claude/hooks` yet. Once the module is live, a hook the template wires but the live file doesn't is a hard `FAIL` again (fleet-config#942).
